...
enum resultado_batalla { BATALLA_GANADA, BATALLA_PERDIDA, BATALLA_ESCAPADA };
...
{
	return tecla_presionada == tecla_buscada ||
	       tecla_presionada == tolower(tecla_buscada);
}
	...
	while (i < cantidad_pokemon && !flag_error_insertar) {
		if (indices_elegidos[i] > 0) {
			struct pokemon *pokemon_elegido =
				tp1_buscar_orden(tp1, i);
			int j = 0;
			while (j < indices_elegidos[i] &&
			       !flag_error_insertar) {
				lista_t *resultado_insertar = lista_insertar(
					objetivos_elegidos, pokemon_elegido);
...
struct pokemon *elegir_contrincante_al_azar(struct juego juego)
{
	size_t indice_elegido = (size_t)rand() % tp1_cantidad(juego.tp1);

	struct pokemon *pokemon_elegido =
		tp1_buscar_orden(juego.tp1, (int)indice_elegido);
...
bool mostrar_fila(void *_columna_actual, void *_paquete_impresion)
{
	...
	struct paquete_impresion *paquete_impresion =
		(struct paquete_impresion *)_paquete_impresion;

	if (paquete_impresion->indice_a_imprimir ==
		    paquete_impresion->juego->jugador.y &&
	    paquete_impresion->cantidad_iteraciones == X_PREDETERMINADO)
		printf(FORMATO_IMPRESION_CASILLA, COLOR_JUGADOR, ICONO_JUGADOR);
	else if (columna_actual[paquete_impresion->indice_a_imprimir] ==
		 CARACTER_PASTO_COMUN)
		printf(FORMATO_IMPRESION_CASILLA, COLOR_PASTO,
		       ICONO_PASTO_COMUN);
	else if (columna_actual[paquete_impresion->indice_a_imprimir] ==
		 CARACTER_PASTO_PELIGROSO)
		printf(FORMATO_IMPRESION_CASILLA, COLOR_PASTO,
		       ICONO_PASTO_PELIGROSO);
	else if (columna_actual[paquete_impresion->indice_a_imprimir] ==
		 CARACTER_PARED)
		printf(FORMATO_IMPRESION_CASILLA, COLOR_PARED, ICONO_PARED);
	else
		printf(FORMATO_IMPRESION_CASILLA, COLOR_PREDETERMINADO,
		       ICONO_ESPACIO_VACIO);
...
void mostrar_mapa(struct juego juego)
{
	...
	paquete_impresion.indice_a_imprimir =
		juego.jugador.y - (MAX_FILAS_PANTALLA / 2);
	paquete_impresion.cantidad_iteraciones = 0;

	for (int i = 0; i < MAX_FILAS_PANTALLA; i++) {
		lista_con_cada_elemento(juego.columnas_tablero, mostrar_fila,
					&paquete_impresion);
...
int mostrar_objetivos(struct juego juego)
{
	...
	lista_iterador_t *iterador =
		lista_iterador_crear(juego.objetivos_elegidos);
	if (iterador == NULL)
		return ERR_MOSTRAR_OBJETIVOS;

	size_t i = 0;
	bool flag_fallo_obtencion = false;
	while (i < lista_cantidad(juego.objetivos_elegidos) - 1 &&
	       !flag_fallo_obtencion) {
		objetivo_actual = lista_iterador_actual(iterador);

		if (objetivo_actual == NULL) {
			flag_fallo_obtencion = true;
		} else {
			printf(FORMATO_IMPRESION_OBJETIVOS,
			       objetivo_actual->nombre);
	...
	if (!flag_fallo_obtencion) {
		objetivo_actual = lista_iterador_actual(iterador);
		printf(FORMATO_IMPRESION_OBJETIVO_FINAL,
		       objetivo_actual->nombre);
	}
...
int mostrar_ui_exploracion(struct juego juego)
{
	...
	if (juego.jugador.pasos_restantes >= 0)
		printf(MSJ_UI_PASOS_RESTANTES, COLOR_JUGADOR,
...
int desplazar_columnas_tablero(struct juego juego, int tecla_presionada)
{
	...
	lista_t *resultado_eliminar =
		lista_eliminar_posicion(juego.columnas_tablero, 0);
	...
	lista_t *resultado_insertar =
		lista_insertar(juego.columnas_tablero, nueva_columna);
	if (resultado_insertar == NULL) {
		free(nueva_columna);
		return ERR_DESPLAZAR_TABLERO;
	}
...
void logica_de_movimiento(struct juego *juego, int tecla_presionada)
{
	if (se_presiono_tecla(tecla_presionada, TECLA_MOVER_DERECHA)) {
		juego->jugador.x++;
		juego->jugador.pasos_restantes--;
		juego->buscando_batalla = true;
	}
	if (se_presiono_tecla(tecla_presionada, TECLA_MOVER_ARRIBA) &&
	    juego->jugador.y != LIMITE_INFERIOR_Y) {
		juego->jugador.y--;
		juego->jugador.pasos_restantes--;
		juego->buscando_batalla = true;
	}
	if (se_presiono_tecla(tecla_presionada, TECLA_MOVER_ABAJO) &&
	    juego->jugador.y != LIMITE_SUPERIOR_Y) {
		juego->jugador.y++;
		juego->jugador.pasos_restantes--;
		juego->buscando_batalla = true;
	}
...
{
	char opciones[CANTIDAD_MAXIMA_OPCIONES] = { TECLA_ELEGIR_PIEDRA,
						    TECLA_ELEGIR_PAPEL,
						    TECLA_ELEGIR_TIJERA };
...
{
	return (eleccion_jugador == TECLA_ELEGIR_PIEDRA &&
		eleccion_pokemon == TECLA_ELEGIR_TIJERA) ||
	       (eleccion_jugador == TECLA_ELEGIR_PAPEL &&
		eleccion_pokemon == TECLA_ELEGIR_PIEDRA) ||
	       (eleccion_jugador == TECLA_ELEGIR_TIJERA &&
		eleccion_pokemon == TECLA_ELEGIR_PAPEL);
}

int identificar_pokemon_por_puntero(const void *pokemon_actual,
				    const void *pokemon_buscado)
{
...
int atrapar_pokemon(struct juego juego)
{
	...
	bool pokemon_esta_presente =
		lista_buscar(juego.objetivos_elegidos,
			     juego.jugador.detalles_batalla.contrincante,
			     identificar_pokemon_por_puntero, &indice_pokemon);

	if (pokemon_esta_presente) {
		struct pokemon *pokemon_atrapado = lista_eliminar_posicion(
			juego.objetivos_elegidos, indice_pokemon);
...
int grabar_batalla(struct juego juego)
{
	...
	batalla_antigua->contrincante =
		juego.jugador.detalles_batalla.contrincante;
	batalla_antigua->turnos_de_duracion =
		juego.jugador.detalles_batalla.turnos_de_duracion;
	batalla_antigua->resultado = juego.jugador.detalles_batalla.resultado;

	lista_t *resultado_insertar =
		lista_insertar(juego.jugador.record_batallas, batalla_antigua);
	if (resultado_insertar == NULL)
		return ERR_GRABAR_BATALLA;
	return SIN_ERRORES;
}
...
{
	printf(MSJ_UI_CONTRINCANTES_BATALLA, COLOR_JUGADOR, NOMBRE_JUGADOR,
	       COLOR_JUGADOR,
	       juego.jugador.detalles_batalla.contrincante->nombre);
	printf(DIVISOR_UI);
	printf(MSJ_UI_PROPUESTA_ELECCION, COLOR_JUGADOR, TECLA_ELEGIR_PIEDRA,
	       COLOR_JUGADOR, TECLA_ELEGIR_PAPEL, COLOR_JUGADOR,
	       TECLA_ELEGIR_TIJERA, COLOR_JUGADOR, TECLA_ESCAPAR);
}

int loop_batallas(struct juego *juego, int tecla_presionada,
		  struct pokemon *contrincante)
{
	...
	if (!jugador_eligio_opcion) {
		...
		if (se_presiono_tecla(tecla_presionada, TECLA_ESCAPAR)) {
			...
			juego->jugador.detalles_batalla.resultado =
				BATALLA_ESCAPADA;
			juego->jugador.detalles_batalla.turnos_de_duracion++;
			juego->jugador.pasos_restantes -=
				DEDUCCION_PASOS_ESCAPE;
	...
	if (jugador_eligio_opcion) {
		...
		if (eleccion_jugador == eleccion_pokemon) {
			...
			sleep(PAUSA_BATALLA);
		} else if (jugador_gano_batalla(eleccion_jugador,
						eleccion_pokemon)) {
			...
			juego->jugador.detalles_batalla.resultado =
				BATALLA_PERDIDA;
			juego->jugador.pasos_restantes -=
				DEDUCCION_PASOS_DERROTA;
		}
	}

	if (batalla_concluida) {
		char *columna_actual = lista_obtener_posicion(
			juego->columnas_tablero, X_PREDETERMINADO);
		if (columna_actual == NULL)
			return ERR_EDITAR_TERRENO;
		columna_actual[juego->jugador.y] = CARACTER_PASTO_COMUN;
...
{
	if (!juego.buscando_batalla)
		return false;

	char *columna_actual = lista_obtener_posicion(juego.columnas_tablero,
						      X_PREDETERMINADO);
	if (columna_actual == NULL)
		return false;

	if (columna_actual[juego.jugador.y] == CARACTER_PASTO_PELIGROSO &&
	    juego.jugador.pasos_restantes > 0 &&
	    lista_cantidad(juego.objetivos_elegidos) > 0 &&
	    decidir_con_probabilidad(PROBABILIDAD_INICIAR_BATALLA))
		return true;
	return false;
}
...
{
	struct batalla *record_batalla =
		(struct batalla *)_record_batalla_actual;

	if (record_batalla->resultado == BATALLA_GANADA)
		printf(MSJ_UI_RECORD_BATALLA, MSJ_RESULTADO_BATALLA_GANADA,
		       COLOR_JUGADOR, record_batalla->contrincante->nombre,
		       record_batalla->turnos_de_duracion);
	else if (record_batalla->resultado == BATALLA_PERDIDA)
		printf(MSJ_UI_RECORD_BATALLA, MSJ_RESULTADO_BATALLA_PERDIDA,
		       COLOR_JUGADOR, record_batalla->contrincante->nombre,
		       record_batalla->turnos_de_duracion);
	else
		printf(MSJ_UI_RECORD_BATALLA_ESCAPADA, COLOR_JUGADOR,
		       record_batalla->contrincante->nombre,
		       record_batalla->turnos_de_duracion);
...
{
	lista_con_cada_elemento(juego.jugador.record_batallas, mostrar_record,
				NULL);
}
...
int logica_del_juego(int tecla_presionada, void *_juego)
{
	...
	if (condiciones_de_corte(*juego, tecla_presionada) ==
	    FLAG_TERMINAR_JUEGO) {
	...
	if (juego->jugador.detalles_batalla.activa) {
		if (loop_batallas(
		    ...
		    SIN_ERRORES) {
			borrar_pantalla();
			printf(MSJ_ERROR_LOOP_BATALLAS);
			return FLAG_TERMINAR_JUEGO;
		}
	} else {
		logica_de_movimiento(juego, tecla_presionada);

		if (desplazar_columnas_tablero(*juego, tecla_presionada) ==
		    ERR_DESPLAZAR_TABLERO) {
			borrar_pantalla();
			printf(MSJ_ERROR_DESPLAZAR_TABLERO);
			return FLAG_TERMINAR_JUEGO;
		}

		if (mostrar_ui_exploracion(*juego) == ERR_MOSTRAR_OBJETIVOS) {
			borrar_pantalla();
			printf(MSJ_ERROR_MOSTRAR_OBJETIVOS);
			return FLAG_TERMINAR_JUEGO;
		}
...
struct juego *juego_crear(tp1_t *tp1)
{
	...
	if (elegir_objetivos(tp1, juego->objetivos_elegidos) ==
	    ERR_ELEGIR_OBJETIVOS) {
		lista_destruir(juego->columnas_tablero);
		lista_destruir(juego->jugador.record_batallas);
	...
	while (i < MAX_COLUMNAS_TABLERO && !flag_error_inicializacion) {
		...
		if (nueva_columna == NULL) {
			...
			if (i == X_PREDETERMINADO)
				nueva_columna[Y_PREDETERMINADO] =
					CARACTER_ESPACIO_VACIO;

			lista_t *resultado_insertar = lista_insertar(
				juego.columnas_tablero, nueva_columna);
			if (resultado_insertar == NULL)
				flag_error_inicializacion = true;
		}
...
void mostrar_reglas()
{
	...
	printf(MSJ_REGLA_1, NOMBRE_JUGADOR, TECLA_MOVER_ARRIBA,
	       TECLA_MOVER_ABAJO, TECLA_MOVER_DERECHA, COLOR_JUGADOR,
	       PASOS_INICIALES);
	printf(ITEM_REGLA, COLOR_JUGADOR);
	printf(MSJ_REGLA_2, COLOR_PREDETERMINADO, ICONO_ESPACIO_VACIO,
	       COLOR_PASTO, ICONO_PASTO_COMUN, COLOR_PASTO,
	       ICONO_PASTO_PELIGROSO);
	printf(ITEM_REGLA, COLOR_JUGADOR);
	...
	printf(ITEM_REGLA, COLOR_JUGADOR);
	printf(MSJ_REGLA_5, PASOS_EXTRA_VICTORIA, DEDUCCION_PASOS_DERROTA,
	       DEDUCCION_PASOS_ESCAPE);
	printf(ITEM_REGLA, COLOR_JUGADOR);
...
void juego_iniciar(struct juego *juego, unsigned int semilla)
{
	...
	if (generar_columnas_iniciales(*juego) ==
	    ERR_GENERAR_COLUMNAS_INICIALES)
		return;
