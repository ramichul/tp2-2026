...
{
	return opcion_1.hotkey == opcion_2.hotkey &&
	       strcmp(opcion_1.nombre, opcion_2.nombre) == 0;
}

int buscar_opcion(const void *_opcion_actual,
		  const void *_opcion_auxiliar_para_busqueda)
{
	opcion_t *opcion_actual = (opcion_t *)_opcion_actual;
	opcion_t *opcion_auxiliar_para_busqueda =
		(opcion_t *)_opcion_auxiliar_para_busqueda;

	if (opciones_son_iguales(*opcion_actual,
				 *opcion_auxiliar_para_busqueda)) {
		return 0;
	}
	return -1;
}
...
menu_t *menu_crear(char *nombre, char hotkey)
{
	...
	nuevo_menu->paquete_personalizacion =
		calloc(1, sizeof(struct paquete_personalizacion));
	...
	for (int i = 0; i < MAX_COLORES; i++)
		nuevo_menu->paquete_personalizacion->colores[i] =
			VALOR_PREDETERMINADO_COLOR;
	...
	lista_t *resultado_insertar =
		lista_insertar(nuevo_menu->listado_opciones, opcion_de_salida);
			    ...
			    menu_t *submenu)
{
	...
	lista_t *resultado_insertar = lista_insertar_posicion(
		menu->listado_opciones,
		lista_cantidad(menu->listado_opciones) - 1, nueva_opcion);
			     ...
			     menu_t **submenu_asociado)
{
	...
	bool opcion_es_valida = lista_buscar(menu->listado_opciones,
					     &opcion_auxiliar_para_busqueda,
					     buscar_opcion, &indice_opcion);
	if (!opcion_es_valida)
		return NULL;

	opcion_t *opcion_a_eliminar =
		lista_obtener_posicion(menu->listado_opciones, indice_opcion);
	if (opcion_a_eliminar == opcion_de_salida)
		return NULL;

	opcion_a_eliminar =
		lista_eliminar_posicion(menu->listado_opciones, indice_opcion);
...
void menu_personalizar_tipografia(menu_t *menu, char *titulo_menu,
				  char *marcador_opcion, char *propuesta_opcion)
{
	if (menu == NULL) {
		return;
	}
	...
	if (marcador_opcion != NULL) {
		menu->paquete_personalizacion->marcador_opcion =
			marcador_opcion;
	}
	if (propuesta_opcion != NULL) {
		menu->paquete_personalizacion->propuesta_opcion =
			propuesta_opcion;
	}
}
...
void agregar_color(struct paquete_personalizacion *paquete_personalizacion,
		   enum componente_menu componente_menu)
{
	if (paquete_personalizacion->colores[componente_menu] !=
	    VALOR_PREDETERMINADO_COLOR) {
		printf(FORMATO_IMPRESION_COLOR,
		       paquete_personalizacion->colores[componente_menu]);
	}
}

void imprimir_con_color(struct paquete_personalizacion *paquete_personalizacion,
			enum componente_menu componente_menu,
			char *formato_impresion, void *componente_1,
			void *componente_2)
{
...
bool imprimir_opcion(void *_opcion_actual, void *_paquete_personalizacion)
{
	opcion_t *opcion_actual = (opcion_t *)_opcion_actual;
	struct paquete_personalizacion *paquete_personalizacion =
		(struct paquete_personalizacion *)_paquete_personalizacion;

	if (paquete_personalizacion->marcador_opcion != NULL) {
		imprimir_con_color(paquete_personalizacion, MARCADOR,
				   FORMATO_IMPRESION_MARCADOR,
				   paquete_personalizacion->marcador_opcion,
				   NULL);
	}

	imprimir_con_color(paquete_personalizacion, OPCION,
			   FORMATO_IMPRESION_OPCION, opcion_actual->nombre,
			   &opcion_actual->hotkey);
...
void mostrar_menu_por_pantalla(menu_t *menu)
{
	...
	if (menu->paquete_personalizacion->titulo_menu != NULL) {
		imprimir_con_color(menu->paquete_personalizacion, TITULO,
				   FORMATO_IMPRESION_TITULO,
				   menu->paquete_personalizacion->titulo_menu,
				   NULL);
	}

	lista_con_cada_elemento(menu->listado_opciones, imprimir_opcion,
				menu->paquete_personalizacion);

	if (menu->paquete_personalizacion->propuesta_opcion != NULL) {
		imprimir_con_color(
			menu->paquete_personalizacion, PROPUESTA_OPCION,
			FORMATO_IMPRESION_MENSAJE_DE_OPCION,
			menu->paquete_personalizacion->propuesta_opcion, NULL);
	}
}

void capturar_hotkeys(menu_t *menu, char *hotkeys_capturadas,
		      size_t *conteo_entradas_capturadas)
{
	...
	do {
		...
		hotkey_es_valida = lista_buscar(menu->listado_opciones,
						&hotkey_ingresada,
						buscar_hotkey, &indice_opcion);
	...
	if (tolower(hotkey_ingresada) == tolower(opcion_de_salida->hotkey)) {
		if (menu->menu_anterior != NULL) {
			...
			menu_iniciar(menu->menu_anterior, hotkeys_capturadas,
				     conteo_entradas_capturadas);
		}
	} else {
		hotkeys_capturadas[*conteo_entradas_capturadas] =
			(char)tolower(hotkey_ingresada);
		(*conteo_entradas_capturadas)++;

		opcion_t *opcion_elegida = lista_obtener_posicion(
			menu->listado_opciones, indice_opcion);
		if (opcion_elegida->submenu != NULL)
			menu_iniciar(opcion_elegida->submenu,
				     hotkeys_capturadas,
				     conteo_entradas_capturadas);
	}
}
...
void menu_iniciar(menu_t *menu, char *hotkeys_capturadas,
		  size_t *conteo_entradas_capturadas)
{
	if (menu == NULL || hotkeys_capturadas == NULL ||
	    conteo_entradas_capturadas == NULL) {
		return;
	}
...
char *menu_leer_entrada(menu_t *menu, char *propuesta_entrada)
{
	...
	imprimir_con_color(menu->paquete_personalizacion, PROPUESTA_OPCION,
			   FORMATO_IMPRESION_PROPUESTA_INGRESO_DE_CADENA,
			   propuesta_entrada, NULL);
...
{
	lista_con_cada_elemento(menu->listado_opciones, destruir_menus,
				&destruir_submenus);
