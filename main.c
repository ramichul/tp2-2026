...
int inicializar_menus(menu_t **menu_principal, menu_t **menu_buscar){
    *menu_principal = menu_crear(NOMBRE_SALIR_PRINCIPAL, HOTKEY_SALIR_PRINCIPAL);
    ...
    *menu_buscar = menu_crear(NOMBRE_SALIR_BUSQUEDA, HOTKEY_SALIR_BUSQUEDA);
    ...
    char *nombres_opciones[MAX_OPCIONES_MENU] = {NOMBRE_JUGAR, NOMBRE_JUGAR_CON_SEMILLA, NOMBRE_BUSCAR, NOMBRE_BUSCAR_NOMBRE, NOMBRE_BUSCAR_TIPO};
    char hotkeys_opciones[MAX_OPCIONES_MENU] = {HOTKEY_JUGAR, HOTKEY_JUGAR_CON_SEMILLA, HOTKEY_BUSCAR, HOTKEY_BUSCAR_NOMBRE, HOTKEY_BUSCAR_TIPO};
    menu_t *submenus[MAX_OPCIONES_MENU] = {NULL, NULL, *menu_buscar, NULL, NULL};
    ...
    while (i < MAX_OPCIONES_MENU  && !flag_fallo_agregar){
        ...
        if (i < CANT_OPCIONES_MENU_PRINCIPAL)
            resultado_agregar = menu_agregar_opcion(*menu_principal, nombres_opciones[i], hotkeys_opciones[i], submenus[i]);
        else
            resultado_agregar = menu_agregar_opcion(*menu_buscar, nombres_opciones[i], hotkeys_opciones[i], submenus[i]);
    ...
    menu_personalizar_tipografia(*menu_principal, TITULO_PRINCIPAL, MARCADOR_OPCION, PROPUESTA_OPCION);
    menu_personalizar_tipografia(*menu_buscar, TITULO_BUSQUEDA, MARCADOR_OPCION, PROPUESTA_OPCION);

    size_t colores[MAX_COLORES] = {COLOR_TITULO, COLOR_OPCIONES, COLOR_MARCADOR, COLOR_PROPUESTA_OPCION};
...
{
	char *nombres_completos_tipos[] = { NOMBRE_TIPO_ELECTRICO,
					  NOMBRE_TIPO_FUEGO,
					  NOMBRE_TIPO_PLANTA,
					  NOMBRE_TIPO_AGUA,
					  NOMBRE_TIPO_NORMAL,
					  NOMBRE_TIPO_FANTASMA,
					  NOMBRE_TIPO_PSIQUICO,
					  NOMBRE_TIPO_LUCHA };
...
bool mostrar_pokemon_por_tipo(struct diccionario_par *par_actual, void *_tipo_buscado){
...
int ejecutar_comando_iniciar_juego(menu_t *menu, char entrada_usuario, tp1_t *tp1){
    ...
    if (entrada_usuario == (char)tolower(HOTKEY_JUGAR)){
        ...
        semilla_leida = menu_leer_entrada(menu, PROPUESTA_INGRESAR_SEMILLA);
...
void ejecutar_comando_buscar_nombre(menu_t *menu, diccionario_t *diccionario){
    char *nombre_buscado = menu_leer_entrada(menu, PROPUESTA_INGRESAR_NOMBRE);

	struct pokemon *pokemon_buscado = diccionario_obtener(diccionario, nombre_buscado);
...
{
	char tipos_validos[MAX_TIPOS][MAX_NOM_TIPO] = { ABREVIACION_ELECTRICO, ABREVIACION_FUEGO, ABREVIACION_PLANTA, ABREVIACION_AGUA, ABREVIACION_NORMAL, ABREVIACION_FANTASMA, ABREVIACION_PSIQUICO,  ABREVIACION_LUCHA };
...
void ejecutar_comando_buscar_tipo(menu_t *menu, diccionario_t *diccionario){
	...
	if (id_tipo != -1)
        cantidad_de_pokemon_mostrados = diccionario_con_cada_elemento(diccionario, mostrar_pokemon_por_tipo, &id_tipo);
...
void ejecutar_comando_busqueda(menu_t *menu, char entrada_usuario, diccionario_t *diccionario){
...
bool usuario_eligio_jugar(char entrada_usuario){
    return entrada_usuario == (char)tolower(HOTKEY_JUGAR) || entrada_usuario == (char)tolower(HOTKEY_JUGAR_CON_SEMILLA);
}

int ejecutar_comandos(menu_t* menu, char entrada_usuario[MAX_PROFUNDIDAD_MENU], size_t conteo_entradas_capturadas, diccionario_t *diccionario, tp1_t *tp1){
    if (usuario_eligio_jugar(entrada_usuario[0])){
        if (ejecutar_comando_iniciar_juego(menu, entrada_usuario[0], tp1) == ERR_INICIAR_JUEGO)
            return ERR_EJECUTAR_COMANDOS;
    }
    else if (conteo_entradas_capturadas == 2){
        ejecutar_comando_busqueda(menu, entrada_usuario[1], diccionario);
    }
...
bool cargar_pokemon_en_diccionario(struct pokemon *pokemon, void *_diccionario){
	...
	return diccionario_insertar(diccionario, pokemon->nombre, pokemon, NULL) == diccionario;
}

int rellenar_diccionario(tp1_t *tp1, diccionario_t *diccionario){
	size_t cantidad_pokemones_insertados = tp1_con_cada_pokemon(tp1, cargar_pokemon_en_diccionario, diccionario);
...
int main(int argc, char *argv[]){
	...
	if (rellenar_diccionario(tp1, diccionario) == ERR_RELLENAR_DICCIONARIO){
    ...
    if (inicializar_menus(&menu_principal, &menu_buscar) == ERR_INICIALIZACION_MENU){
    ...
    menu_iniciar(menu_principal, entrada_usuario, &conteo_entradas_capturadas);

    if (ejecutar_comandos(menu_principal, entrada_usuario, conteo_entradas_capturadas, diccionario, tp1) == ERR_EJECUTAR_COMANDOS)
        return ERR_EJECUTAR_COMANDOS;
